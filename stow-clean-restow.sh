...
if [ "$#" -gt 0 ]; then
  ...
  PACKAGES=(shell sway waybar ghostty tmux env hyprland niri)
fi
...
for bin in run-swayidle run-niri-swayidle sway-handle-lid.sh niri-handle-lid.sh hypr-handle-lid.sh hypr-kblayout-waybar hypr-move-workspace-next-output wifi-menu obsidian-launch theme-toggle graphify-estimate-cost rofi-bluetooth; do
