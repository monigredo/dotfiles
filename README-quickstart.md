...
This installs packages, fonts, git defaults, creates `~/code`, and **prompts for git user.name/email**. If no global Git email is set, the email prompt defaults to `8849111+monigredo@users.noreply.github.com`. It also prompts to set `zsh` as your login shell, installs \`rofi-bluetooth\` from GitHub, and links it into \`~/.local/bin\` for you:
